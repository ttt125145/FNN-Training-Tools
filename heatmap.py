...
# 加载数据
start_seed = 1
accuracies_arr = np.zeros((80,250))
for c in range(80):
    print(c)
...
plt.title('aver accuracies change among batch_size')
y = np.array([10,20,40,80,160])
x = np.array(range(40, 2001, 40))
for i in range(5):
    bzs = average_nlbz_arr[i,:]
    plt.plot(x,bzs,label=f'n={y[i]}')
plt.legend()
plt.savefig('aver_accu_plot_in_different_n.png')
plt.close()
...
# heatmap = plt.imshow(average_nlbz_arr,  
#                      origin='lower',
#                      aspect='auto',
#                      extent=extent,
#                      cmap=color_map)
...
# plt.title('Average accuracies under different batch_size and n')
...
# # 保存图像
# plt.savefig('aver_nlbz_heatmap.png', dpi=300, bbox_inches='tight')
# plt.close()