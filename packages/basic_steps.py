...
def preprocess():
    ...
    返回:
    transform: torchvision.transforms.Compose - 预处理步骤的组合
    """
...
def build_loader_by_batchsize(train_dataset, test_dataset, batchsize=40):
    """
    根据batch size创建数据集加载器

    参数:
    train_dataset: torch.utils.data.Subset - 训练集子集
    test_dataset: torch.utils.data.Subset - 测试集子集
...
def train_one_epoch(device, model, optimizer, criterion, train_loader):
    ...
    参数:
    device: torch.device - 当前使用的设备
    model: nn.Module - 需要训练的神经网络模型
    optimizer: torch.optim.Optimizer - 优化器实例
    criterion: torch.nn.Module - 损失函数
    train_loader: torch.utils.data.DataLoader - 训练集加载器

    返回:
    running_loss: float - 本次epoch的总损失
    total_train: int - 本次epoch的总训练样本数
    correct_train: int - 本次epoch的正确训练样本数
    layer_output: torch.Tensor - 最后一批次的隐藏层输出
    """
...
def test_one_epoch(device, model, test_loader):
    ...
    参数:
    device: torch.device - 当前使用的设备
    model: nn.Module - 需要测试的神经网络模型
    test_loader: torch.utils.data.DataLoader - 测试集加载器

    返回:
    test_accuracy: float - 本次epoch的测试准确率
    """
...
"""依次传入: 设备，模型，优化器，损失函数，训练集,测试集,种子,data_path;"""
"""传出数据记录："""


def run_one_epoch(
    device, model, optimizer, criterion, train_loader, test_loader, seed, data_path
):
    """
    运行一个epoch的训练和测试
    ...
    参数:
    device: torch.device - 当前使用的设备
    model: nn.Module - 需要训练和测试的神经网络模型
    optimizer: torch.optim.Optimizer - 优化器实例
    criterion: torch.nn.Module - 损失函数
    train_loader: torch.utils.data.DataLoader - 训练集加载器
    test_loader: torch.utils.data.DataLoader - 测试集加载器
    seed: int - 随机种子，用于结果保存命名
    data_path: str - 结果保存路径

    返回:
    train_loss: float - 本次epoch的训练损失
    train_accuracy: float - 本次epoch的训练准确率
    test_accuracy: float - 本次epoch的测试准确率
    layer_output: torch.Tensor - 最后一批次的隐藏层输出
    dt: float - 本次epoch的运行时间（秒）
    """

    t1 = time.time()
    best_accuracy = 0.0  # 记录最佳验证集准确率
    # 训练
    running_loss, total_train, correct_train, layer_output = train_one_epoch(
    ...
    if test_accuracy > best_accuracy:
        ...
        torch.save(
            model.state_dict(),
            os.path.join(data_path, "best_models", f"best_model(seed{seed}).pth"),
        )
    t2 = time.time()
...
def one_simulation(
    ...
    for epoch in range(epochs):
        train_loss, train_accuracy, test_accuracy, layer_output, dt = run_one_epoch(
            device,
            model,
            optimizer,
            criterion,
    ...
    np.savez(
        os.path.join(
            data_path,
            "loss_accuracies_during_epoch",
            f"loss_accuracies(seed{seed}).npz",
        ),
        **summaries,
    )
    ...
    np.savez(
        os.path.join(
            data_path, "dinamic_weights", f"dyn_weights_array(seed{seed}).npz"
        ),
        **formated_wei,
    )
    ...
    formated_bi = {}
    for k in bias.keys():
        formated_bi[k] = [zd[k] for zd in biases]
    for k, b_ls in formated_bi.items():
        formated_bi[k] = np.stack([b.cpu().numpy() for b in b_ls])
    np.savez(
        os.path.join(data_path, "dinamic_bias", f"dyn_bias_array(seed{seed}).npz"),
        **formated_bi,
    )
    ...
    np.save(
        os.path.join(
            data_path,
            "dinamic_layers_out",
            f"layers_out(seed{seed})_shape{layers_epo_array.shape}.npy",
        ),
        layers_epo_array,
    )
