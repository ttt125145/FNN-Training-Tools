...
def almost_prepare(device,nl=10,num_hidden_layers=2,batchsize=40,criterion=nn.MultiMarginLoss(),Optimizer=optim.SGD):
    train_loader,test_loader = preprocess_loader(batchsize)
    model = build_flexible_FNN(nl,num_hidden_layers,device)
    optimizer = Optimizer(model.parameters(),lr=0.01)
    return device,model,optimizer,criterion,train_loader,test_loader
...
'''传出数据记录：'''        
def run_one_epoch(device,model,optimizer,criterion,train_loader,test_loader,seed,data_path):
    t1 = time.time()
...
'''功能2:返回 最后准确率/最高准确率'''
def one_simulation(device,model,optimizer,criterion,train_loader,test_loader,epochs,seed,data_path):
    t1 = time.time()
