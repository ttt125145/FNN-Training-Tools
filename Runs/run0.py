...
def __main__():
    ...
    for c in range(copy_num):
        ...
        for i, j in np.ndindex(
            ...
            nl, bz = int(pot_arr[i, j, 0]), int(
                pot_arr[i, j, 1]
            )  # np.Int32转化为python内置int类型
            device, model, optimizer, criterion, train_loader, test_loader = (
                bs.almost_prepare(  # 此括号传入循环的参数，以匹配项目要求
                    device=device,
                    nl=nl,
                    num_hidden_layers=2,
                    batchsize=bz,
            ...
            print(
                f"分组区间[{xuhao*copy_num},{(xuhao+1)*copy_num})，复本{xuhao*copy_num+c}({(i*pot_arr.shape[1]+j+1)}/{models_num}),model(nl{nl},bz{bz}),用时{dt/60:.2f}分\
        ...
        print(
            f"[{xuhao*copy_num},{(xuhao+1)*copy_num}) 复本{xuhao*copy_num+c}({c+1}/{copy_num}),用时：{ddt/3600:.2f}小时,\
